...
<#
    Bootstrap proxy setup for servers that must use a proxy before REST calls are made.
    Replace Placehold below with your proxy address, for example: http://proxy.company.com:8080
    This is placed at the top so REST calls can use the proxy immediately.
    The environment proxy loaded later from .\env\dev.env or .\env\prod.env can still override this value.
#>
...
if ($proxyUri -and $proxyUri.Trim() -notmatch '^(Placehold|Placeholder|YOUR-|<.*>|CHANGEME)$') {
    ...
    # $true bypasses proxy for local addresses.
    $proxy = New-Object System.Net.WebProxy($proxyUri, $true)

    # Assign it as the default for WebRequest.
    [System.Net.WebRequest]::DefaultWebProxy = $proxy

    # Optional: use the logged-in Windows credentials when the proxy requires domain authentication.
    [System.Net.WebRequest]::DefaultWebProxy.Credentials = [System.Net.CredentialCache]::DefaultNetworkCredentials

    # Optional: verify the proxy address without exposing credentials.
...
<#
    ...
    This keeps placeholder env values from being treated as real configuration.
#>
...
<#
    Creates the logs and reports folders when needed.
    Starts a new timestamped log and report file for this run.
#>
function New-RunLog {
...
<#
    ...
    Also prints the same message to the console.
#>
...
<#
    Reads simple KEY=VALUE entries from an env file.
...
<#
    Merges multiple env maps.
    Later files win, so dev.env/prod.env can override shared .env values.
#>
...
<#
    Normalizes whatever PVWA value is supplied.
    This makes sure all REST calls can append PasswordVault/API paths consistently.
#>
...
<#
    Turns TENANT_ID into a full CyberArk Identity base URL.
    Accepts tenant short name or full URL.
#>
...
<#
    Builds the proxy URL from PROXY or PROXY_ADDRESS/PROXY_PORT values.
    Also supports the ROXY_ADDRESS typo from the original sample.
#>
...
<#
    Loads shared and environment-specific settings.
    Builds runtime URLs.
    Applies proxy settings.
    Stores final configuration in script scope.
#>
function Import-RunConfig {
    ...
    # Values in dev.env/prod.env override .env.
    ...
    # Apply env-based proxy if present.
    # This can override the proxy configured at the top of the script.
...
<#
    Converts a hashtable into application/x-www-form-urlencoded text.
    Used for the Identity token request.
#>
...
<#
    Creates a SHA256 hash of the bearer token for logging.
    This is for traceability only and cannot be used to authenticate.
#>
...
<#
    Extracts useful response-body text from failed REST calls.
    This makes API errors easier to troubleshoot.
#>
function Get-WebErrorText {
    ...
    if ($null -ne $response) {
        try {
            ...
            # Keep original error if response body cannot be read.
        }
    }
...
<#
    Prompts for the service-user credential.
    Requests the Identity bearer token.
    Logs only a fingerprint of the token.
#>
function Get-IdentityBearerToken {
    ...
    # Keep the real token in memory only.
    # Log only a fingerprint for troubleshooting correlation.
...
<#
    Central wrapper for CyberArk REST calls.
    Builds the URL, adds the bearer token, serializes JSON, applies proxy settings, and normalizes errors.
#>
function Invoke-CyberArkRest {
    ...
    # String bodies are already serialized or form encoded.
    if ($null -ne $Body) {
        if ($Body -is [string]) {
            $params['Body'] = $Body
        }
...
<#
    ...
    Removes empty rows.
    Confirms an AccountID column is present.
    Returns all rows for processing.
#>
...
<#
    Reads a value from a CSV row using several accepted column aliases.
    This keeps the CSV format flexible.
#>
...
<#
    Collects platform account properties from CSV columns named:
      Prop:<name>
      Property:<name>
...
<#
    Loads regular CyberArk platforms once at the beginning of the run.
    Every account row uses this local cache for validation.
#>
function Get-PlatformInventory {
    ...
    # Only regular platforms are loaded because this script updates vaulted account platforms,
    # not group or dependent platform types.
...
<#
    Performs the core platform validation:
      - source platform exists
      - target platform exists
      - target platform is active
      - target platform is regular
      - source and target platform system/device type match
      - target Safe is allowed by platform AllowedSafes
#>
...
<#
    Combines current platform account properties with CSV-supplied properties.
    CSV values override current values.
#>
...
<#
    Builds the JSON Patch operation array for same-Safe updates.
    Only fields that actually need to change are added.
#>
function New-PatchBody {
    ...
    # Build a JSON Patch array.
    # Each entry represents one field update sent to PATCH /Accounts/{id}/.
    $ops = @()
    ...
    # Platform properties are sent as one object.
    # Existing properties not included here remain unchanged by CyberArk.
...
function New-AccountCreateBody {
    ...
    # New-account create body.
    # The secret is required because the public REST API creates a new account
    # rather than performing the UI move operation.
    $body = [ordered]@{
        name = $name
        address = $address
        userName = $userName
...
<#
    Handles the create side of a Safe change.
    Retrieves the secret, builds the new account body, and creates the target account.
#>
function Invoke-SafeMoveCreate {
    ...
    # Safe change is intentionally gated because this path retrieves the current secret
    # to create the target account.
...
<#
    Optionally deletes the source account after target creation.
    It only deletes when both delete-confirmation switches are supplied.
#>
function Remove-SourceAccountAfterMove {
    ...
    if (-not $DeleteSourceAfterSafeMove) {
        return 'Source account kept. DeleteSourceAfterSafeMove was not supplied.'
    }

    if ($ConfirmDeleteSourceAfterMove -ne 'YES') {
        return 'Source account kept. ConfirmDeleteSourceAfterMove was not YES.'
    }
...
<#
    Processes one CSV row end to end:
      1. read account
      2. validate target platform/Safe
      3. choose patch versus safe-change create
      4. return result
#>
function Invoke-RowUpdate {
    ...
    # Prepare a standard report row before doing any update.
    # This guarantees every CSV row gets a result.
    $result = [ordered]@{
        Row = $RowNumber
        AccountID = $accountId
        SourceSafe = ''
    ...
    try {
        ...
        # AutomaticManagementEnabled and DisableAutomaticManagement are optional.
        # Null means leave current CPM management setting unchanged.
